...
### Added
- WCAG contrast-aware colors: NWS and MeteoAlarm theme alerts boost foreground fills for individual events whose raw color reads poorly against the active theme's card background, and only in the direction where it fails. Two tiers: a text tier (~2:1) darkens the icon, label, countdown, source link, and compact accent; a stricter progress tier (~1.3:1) re-tints the progress-bar fill, which carries no weight and only needs help when the raw hue is near-invisible (yellow Tornado Watch, cyan Freeze Watch). Tornado Warning and other colors that already pass both tiers render unchanged; pale hues like Severe Thunderstorm Warning are boosted toward the text color on light themes only, while dark hues like Freeze Warning and Flash Flood Warning are boosted on dark themes only. The NWS event color table was backfilled from weather.gov/help-map (110+ event types) and boost tags are precomputed at build time so the runtime cost is one lookup. New `enhanceContrast` config (default `true`) — set to `false` to restore raw colors everywhere.
